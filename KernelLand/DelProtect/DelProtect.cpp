 ...
 * Routine Description:
 * 
 *	This is the unload routine for this minFilter driver. This is called
 ...
 * Arguments:
 * 
 *	Flags - Indicating if this is mandatory unload.
 * Return Value:
 * 
 ...
 * 
 *	This routine is non-pageable.
 * 
 * Arguments:
 *	Data - Pointer to the filter callback data that is passed to us.
 * 
 *	FltObjects - Pointer to the FLT_RELATED_OBJECTS data structure containing
 *		opaque handles to this filter, instance, its associtated volume and 
 *		file object.
 * 
 ...
 *
 *	This is a non-pageable routine.
 *
 * Arguments:
 *
 *	FltObjects - Pointer to the FLT_RELATED_OBJECTS data structure containing
 ...
 *		file objects.
 * 
 *	RequesterConstex - The context for the completion routine for this
 *		operation.
 *
 *	Operation Status -
 *
 ...
 * *	Data - Pointer to the filter callback data that is passed to us.
 *
 *	FltObjects - Pointerto the FLT_RELATED_OBJECTS data structure containing
 *		opaque handles to this filter, instance, its associated volume and
 *		file object.
 *
 *	CompletionContext - The context for the completion routine for this
 ...
 */
{
	...
	/*
	 * This template code does not do anything with the callback data, but
...
)
{
	...
	if (params.Options & FILE_DELETE_ON_CLOSE)
	{
		...
		if (!IsDeleteAllowed(PsGetCurrentProcess()))
		{
			...
			KdPrint(("Prevent delete from IRP_MJ_CREAYE by cmd.exe\n"));
		}
	}
...
)
{
	...
	if (!IsDeleteAllowed(process))
	{
		Data->IoStatus.Status = STATUS_ACCESS_DENIED;
		returnStatus = FLT_PREOP_COMPLETE;
		KdPrint(("Prevent delete from IRP_MJ_SET_INFRORMATION by cmd.exe\n"));
	}
...
)
{
	...
	auto processName = (UNICODE_STRING*)ExAllocatePool2(POOL_FLAG_PAGED, size,
		DRIVER_TAG);
	...
	if (processName)
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			if (processName->Length > 0 &&
				wcsstr(processName->Buffer, L"\\System32\\cmd.exe") != nullptr ||
